...
#         except Exception:
#             # if reading fails (corrupt or absent engine), just overwrite
#             out = df_row
#     else:
#         out = df_row
...
#     # Console report (fractions; multiply by 100 when formatting if you want %)
#     print("\n=== WiC Evaluation ===")
...
def run_once(df, infer, template, model, provider, args, run_id):
    """Run evaluation one time and return metrics + predictions."""
    preds, golds, times, detailed_rows = [], [], [], []
    ...
    results_df = pd.DataFrame(detailed_rows)
    results_df["correct"] = results_df["gold"] == results_df["pred"]
    csv_output_path = f"../../results_evaluation/csv_results/{args.outfile.replace('.csv','')}_run{run_id}.csv"
    results_df.to_csv(csv_output_path, index=False, encoding="utf-8")
    print(f"Saved CSV predictions to: {csv_output_path}")

    # Compute metrics
    met = compute_metrics(golds, preds)
    met["avg_latency"] = np.mean(times) if times else 0.0
    return met


def main():
    base_dir = os.path.dirname(os.path.abspath(__file__))
    metrics_results_path = "../../results_evaluation/metrics_new_log.xlsx"
    ...
    ap.add_argument("--provider", required=True,
                    choices=["openai", "gemini", "ollama"])
    ap.add_argument("--model", default=None,
                    help="openai(gpt-4o), gemini(gemini-1.5-pro|gemini-1.5-flash), ollama(llama3).")
    ap.add_argument("--mode", choices=["zero","few"], default="few",
                    help="Prompt mode.")
    ap.add_argument("--lang", choices=["kk","en"], default="kk",
                    help="Prompt language.")
    ap.add_argument("--limit", type=int, default=None,
                    help="Evaluate first N items only.")
    ap.add_argument("--outfile", default="predictions.csv",
                    help="Base name for CSV predictions.")
    ap.add_argument("--metrics_xlsx", default=metrics_results_path,
                    help="Excel file to append one row of metrics for this run.")
    ...
    for run_id in range(1, args.runs + 1):
        ...
        # shuffle data with seed to create variation
        random.seed(run_id)
        df_shuffled = df.sample(frac=1, random_state=run_id).reset_index(drop=True)

        met = run_once(df_shuffled, infer, template, model, args.provider, args, run_id)
    ...
    metrics_row = {
        ...
        "cohens_kappa_mean": mean_metrics["cohens_kappa"],
        "cohens_kappa_std": std_metrics["cohens_kappa"],
        "macro_f1_mean": mean_metrics["macro_f1"],
        "macro_f1_std": std_metrics["macro_f1"],
        "weighted_f1_mean": mean_metrics["weighted_f1"],
        "weighted_f1_std": std_metrics["weighted_f1"],
        "avg_latency_mean": mean_metrics["avg_latency"],
        "avg_latency_std": std_metrics["avg_latency"]
    }

    append_metrics_row(args.metrics_xlsx, metrics_row)
    print(f"\nAppended aggregated metrics to: {args.metrics_xlsx}")
